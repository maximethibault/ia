...
.EXAMPLE
    Connect-AzureARMVM -AzureSubscriptionId "1019**********************" -ResourceGroupName "RG1" -VMName "VM01" -AzureOrgIdCredentialName $cred
...
#        [String]$AzureOrgIdCredentialName,
 ...
 #   $IpAddress = InlineScript{

 #   $ErrorActionPreference = "SilentlyContinue"
 #   $Login = Login-AzureRmAccount -Credential $using:AzureOrgIdCredential
...
$$ResourceGroupName = ”iAPOCAAutomationRS”
	# Select the Azure subscription we will be working against
#    $selectSubscription = Select-AzureRmSubscription -SubscriptionId $using:AzureSubscriptionId
...
$InboundRule = $nsg | Add-AzureRmNetworkSecurityRuleConfig -Name "WinRM_HTTPS" -Priority 1100 -Protocol TCP -Access Allow -SourceAddressPrefix $SourceAddressPrefix -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 5986 -Direction Inbound -ErrorAction SilentlyContinue | Set-AzureRmNetworkSecurityGroup -ErrorAction SilentlyContinue
        ...
        Configure-AzureWinRMHTTPS -ResourceGroupName $using:ResourceGroupName -VMName $using:VMName

        $VM = Get-AzureRmVM -ResourceGroupName $using:ResourceGroupName -Name $using:VMName
        if($VM)
        {
